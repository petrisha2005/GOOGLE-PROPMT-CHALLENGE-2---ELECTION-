...
const optionsHI: Record<string, Record<string, string[]>> = {
  ...
  'usa_deadlines': {
    ...
    'usa-ballot-tracking': [
      'अपने राज्य के ऑनलाइन "वेयर इज़ माई बैलट" ट्रैकिंग सिस्टम का उपयोग करें',
      'बिना जांच किए मान लें कि यह सुरक्षित रूप से पहुंच गया है'
    ]
  },
  'estonia_ivoting': {
    'ee-digital-id': [
    ...
    'ee-download-app': [
      'एक सार्वजनिक वेब फोरम पर',
      'राष्ट्रीय चुनावी साइट से आधिकारिक ई-वोटिंग एप्लिकेशन डाउनलोड करें'
    ],
    'ee-enter-pin1': [
  ...
  'australia_ranked': {
    ...
    'au-house-preferential': [
      'अपनी पहली पसंद में \'१\' लिखें, और अन्य सभी बॉक्स में क्रमिक संख्याएं \'२\', \'३\', आदि लिखें',
      'केवल एक बॉक्स में एक क्रॉस का निशान लगाएं'
    ],
    'au-senate-voting': [
      'क्रम में कम से कम 6 राजनीतिक दलों को नंबर देकर \'लाइन के ऊपर\' वोट करें, या कम से कम 12 उम्मीदवारों को नंबर देकर \'लाइन के नीचे\' वोट करें',
      'नीचे अपना नाम हस्ताक्षर करें'
    ],
    'au-deposit-sausage': [
...
export default function JourneySimulator() {
  ...
  const [activeTrackId, setActiveTrackId] = useState<string>('india_evm');
  ...
  return (
    <div className="glass-panel rounded-3xl p-6 md:p-8 border border-slate-850 space-y-8 shadow-2xl relative overflow-hidden">
      ...
      {/* NEW TRACK SELECTOR BAR */}
      <div className="bg-[#0B192C]/75 border-2 border-slate-850 rounded-2xl p-2 relative z-10 grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
      ...
      {/* STUNNING CONFETTI / SCORECARD COMPLETION OVERLAY */}
      {isComplete ? (
        <div className="bg-[#0B192C]/90 backdrop-blur-md rounded-2xl border border-slate-800 p-8 text-center space-y-6 animate-scaleIn flex flex-col items-center justify-center min-h-[420px] relative z-20">
        ...
        <>
          ...
          >
            ...
            <div className="hidden md:block absolute top-1/2 inset-x-12 h-2 bg-slate-900 border border-slate-850 rounded-full -translate-y-1/2 -z-15 shadow-inner">
              <div 
            ...
            <div className="md:hidden absolute left-1/2 inset-y-10 w-2 bg-slate-900 border border-slate-850 rounded-full -translate-x-1/2 -z-15 shadow-inner">
              <div 
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* Completion Checklist checkmark */}
                    {isCompleted && (
          ...
          {/* DYNAMIC CHALLENGE AREA WITH STEP 5 EVM INTERACTIVE SPLIT OVERRIDE */}
          {currentStage && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
              
              {/* CHALLENGE CONSOLE (Left side or Full-width) */}
              <div className={`space-y-6 ${currentStepIndex === 4 && activeTrackId === 'india_evm' ? 'lg:col-span-6' : 'lg:col-span-12'} animate-scaleIn`}>
                
                {/* INTERACTIVE CHALLENGE CONSOLE CONTAINER */}
                <div className="bg-[#0B192C]/40 border-2 border-slate-850 p-6 rounded-3xl relative overflow-hidden shadow-inner space-y-5">
                  ...
                  <div className="p-5 bg-slate-950/70 border border-slate-850 rounded-2xl flex items-start gap-3.5 shadow-inner">
                    ...
                    {currentStage.options.map((_, idx) => {
                      ...
                      if (hasAnswered) {
                        ...
                        if (isCorrectOption) {
                          ...
                          cardClasses = 'border-slate-850 bg-slate-950/50 text-slate-600 opacity-60';
                        }
                      }
                      ...
                      return (
                        ...
                        >
                          <span className={`text-[9px] font-black uppercase tracking-widest font-outfit px-2 py-0.5 rounded-full border mb-3 w-fit ${
                            isSelected ? 'bg-civic-gold/10 border-civic-gold/25 text-civic-gold' : 'bg-slate-900 border-slate-850 text-slate-500'
                          }`}>
              ...
              {/* STEP 5 SPECIAL OVERRIDE: THE HIGH-FIDELITY ECI EVM SIMULATION BOX */}
              {currentStepIndex === 4 && activeTrackId === 'india_evm' && (
                <div className="lg:col-span-6 animate-scaleIn flex flex-col justify-center">
                  <div className="bg-slate-350 border-4 border-slate-400 p-4 sm:p-6 rounded-3xl shadow-2xl relative overflow-hidden text-slate-850 space-y-4 max-w-md mx-auto w-full">
                    
                    {/* EVM Top Status Panel */}
                    <div className="flex items-center justify-between border-b-2 border-slate-450 pb-3">
                      <div className="flex items-center gap-2">
                        <div className="w-3.5 h-3.5 rounded-full bg-emerald-500 ring-4 ring-emerald-500/30 shadow-md" />
                        <span className="text-[9px] font-black text-slate-755 uppercase tracking-widest font-outfit">EVM ACTIVE / READY</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[8px] font-black text-slate-655 uppercase tracking-widest block font-outfit">ECI BU-M2</span>
                      </div>
                    </div>

                    {/* EVM Candidates Balloting Column */}
                    <div className="space-y-2.5">
                      {[
                        { id: 'mango', num: 1, name: 'Rajesh Kumar', icon: MangoIcon, color: 'text-yellow-600' },
                        { id: 'umbrella', num: 2, name: 'Priya Sharma', icon: Umbrella, color: 'text-pink-600' },
                        { id: 'bicycle', num: 3, name: 'Amit Patel', icon: Bicycle, color: 'text-teal-600' },
                        { id: 'sun', num: 4, name: 'Dr. Sunita Rao', icon: Sun, color: 'text-amber-600' }
                      ].map((cand) => {
                        ...
                        return (
                          <div 
                            ...
                            className="bg-white border-2 border-slate-400 rounded-xl p-3 flex items-center justify-between shadow-sm cursor-pointer select-none hover:bg-slate-50 transition-colors"
                          >
                            {/* Serial Number & Name */}
                            <div className="flex items-center gap-2.5 shrink-0">
                              <span className="text-xs font-black font-outfit text-slate-500 bg-slate-100 border border-slate-200 w-6 h-6 flex items-center justify-center rounded-lg">{cand.num}</span>
                              <span className="text-xs font-bold text-slate-900 font-outfit truncate max-w-[100px] sm:max-w-none">{cand.name}</span>
                            </div>

                            {/* Party Symbol (Low-Literacy Layout) */}
                            <div className={`p-1 bg-slate-50 border border-slate-200 rounded-lg shrink-0 ${cand.color} hover:scale-105 transition-transform`}>
                              <CandIcon className="w-8 h-8 stroke-[2]" />
                            </div>

                            {/* LED and Tactile Blue Button Panel */}
                            <div className="flex items-center gap-3">
                              {/* LED Light Indicator */}
                              <div className={`w-3.5 h-3.5 rounded-full border border-slate-350 transition-all ${
                                isVoted 
                              ...
                              <button
                                ...
                                className="w-10 h-10 bg-blue-600 border-b-4 border-blue-800 rounded-lg shadow-md hover:bg-blue-500 active:scale-95 transition-all outline-none flex items-center justify-center cursor-pointer disabled:bg-slate-400 disabled:border-slate-500 disabled:cursor-not-allowed"
                                aria-label={`Cast practice vote on candidate ${cand.num}`}
                              >
                                <span className="w-4 h-4 bg-blue-400 rounded-full opacity-40 shadow-inner" />
                              </button>
                            </div>
                          </div>
                        );
          ...
          {/* FOOTER AREA: FEEDBACK BANNER & NAVIGATION CONTROL */}
              ...
              <div className={`p-5 rounded-2xl border flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4 shadow-md ${
                ...
                <div className="flex items-center sm:items-start gap-3">
                  ...
                  <div className="space-y-1 text-center sm:text-left">
                    ...
                    <p className="text-xs text-slate-450 leading-relaxed font-outfit max-w-xl">
                      {isCorrect ? getEncouragingMessage().desc : getRetryMessage().desc}
                    </p>
                  </div>
                </div>

                {/* Incorrect Option Retry Button */}
                {!isCorrect && (
                  <button
